...
const ConsumerScreen = ({ navigation }) => {
  ...
  const handleBarCodeRead = async ({ data }) => {
    ...
    try {
      ...
      if (!batchId) throw new Error('Missing batchId in QR code');
    } catch (e) {
      Alert.alert('Invalid QR Code', `Could not read batch data.\n\n"${data}"`);
      setScanning(false);
      return;
    }
    ...
    try {
      ...
      Alert.alert('Verification Failed', error.message || 'Could not verify this product.');
    } finally {
      setScanning(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Scan the QR code on your food product to view its complete journey from farm to your table
        </Text>
      ...
      <Modal visible={showScanner} animationType="slide" onRequestClose={() => setShowScanner(false)}>
        <View style={styles.scannerContainer}>
          ...
          {/* FIX: overlay outside CameraView */}
      ...
      <Modal visible={showResult} animationType="slide" onRequestClose={() => setShowResult(false)}>
        <View style={styles.resultContainer}>
          ...
          <ScrollView style={styles.resultContent}>
            {verificationResult && (
              <>
                <View style={[styles.authenticityBadge, { backgroundColor: verificationResult.isAuthentic ? '#4CAF50' : '#F44336' }]}>
                  <Icon name={verificationResult.isAuthentic ? 'verified' : 'error'} size={48} color="#fff" />
                  <Text style={styles.authenticityText}>
                    {verificationResult.isAuthentic ? 'Product Verified ✓' : 'Not Authentic ✗'}
                  </Text>
                </View>
                ...
                <View style={styles.resultSection}>
                  ...
                  {[
                    ['Batch ID', verificationResult.batchId],
                    ['Product', verificationResult.productType],
                    ['Producer', verificationResult.producer],
                    ['Quantity', verificationResult.quantity],
                    ['Production', verificationResult.productionDate],
                    ['Expiry', verificationResult.expiryDate],
                    ['FSSAI', verificationResult.fssaiLicense],
                  ].map(([label, value]) => (
                ...
                <View style={styles.resultSection}>
                  <Text style={styles.resultSectionTitle}>
                    Product Journey ({verificationResult.checkpoints?.length || 0} checkpoints)
                  </Text>
                  {verificationResult.checkpoints?.length === 0 ? (
                    <Text style={{ color: '#999', textAlign: 'center', paddingVertical: 16 }}>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: 'bold', color: '#333', marginTop: 24, textAlign: 'center' },
  subtitle: { fontSize: 16, color: '#666', marginTop: 12, textAlign: 'center', lineHeight: 24 },
  scanButton: { backgroundColor: '#366d80ff', flexDirection: 'row', alignItems: 'center', gap: 12, padding: 20, borderRadius: 12, marginTop: 32, elevation: 4 },
  scanButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  features: { flexDirection: 'row', marginTop: 48, gap: 16 },
  feature: { flex: 1, alignItems: 'center', padding: 16, backgroundColor: '#fff', borderRadius: 12, elevation: 2 },
  featureTitle: { fontSize: 14, fontWeight: 'bold', color: '#333', marginTop: 12 },
  featureText: { fontSize: 12, color: '#666', marginTop: 4, textAlign: 'center' },
  ...
  resultHeader: { backgroundColor: '#366d80ff', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16 },
  resultHeaderTitle: { fontSize: 20, fontWeight: 'bold', color: '#fff' },
  resultContent: { flex: 1, padding: 16 },
  authenticityBadge: { padding: 32, borderRadius: 16, alignItems: 'center', marginBottom: 24 },
  authenticityText: { fontSize: 24, fontWeight: 'bold', color: '#fff', marginTop: 16 },
  resultSection: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 16 },
  resultSectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 16 },
  resultRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#f5f5f5' },
  resultLabel: { fontSize: 14, color: '#666', fontWeight: '600' },
  resultValue: { fontSize: 14, color: '#333' },
  checkpointItem: { flexDirection: 'row', marginBottom: 20 },
  checkpointIndicator: { alignItems: 'center', marginRight: 16 },
  checkpointDot: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#366d80ff' },
  checkpointLine: { width: 2, flex: 1, backgroundColor: '#366d80ff', marginTop: 4 },
  checkpointDetails: { flex: 1 },
  checkpointLocation: { fontSize: 16, fontWeight: 'bold', color: '#333' },
  checkpointTime: { fontSize: 12, color: '#666', marginTop: 4 },
  checkpointStatusBadge: { backgroundColor: '#E8F5E9', alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12, marginTop: 8 },
  checkpointStatus: { fontSize: 12, color: '#366d80ff', fontWeight: '600' },
});
