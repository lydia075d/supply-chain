...
const ProducerDashboard = ({ navigation }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'At Warehouse': return '#2196F3';
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={loadBatches} />}>
        ...
        <View style={styles.batchesContainer}>
          ...
          {loading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>Tap the + button to create your first batch</Text>
            </View>
          ) : (
            batches.map(batch => (
          ...
          <View style={styles.qrModalContent}>
            ...
            {selectedBatch && (
              <>
                ...
                <Text style={styles.qrProductType}>{selectedBatch.productType}</Text>
                <Text style={styles.qrQuantity}>{selectedBatch.quantity} kg</Text>
              </>
            )}
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#4fadd9ff', padding: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  batchId: { fontSize: 18, fontWeight: 'bold', color: '#366d80ff' },
  statusBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  statusText: { fontSize: 12, fontWeight: 'bold' },
  batchInfo: { gap: 8 },
  infoRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  infoText: { fontSize: 14, color: '#666' },
  batchFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#eee' },
  checkpointInfo: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  checkpointText: { fontSize: 12, color: '#366d80ff', fontWeight: '600' },
  qrButton: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#E8F5E9', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
  qrButtonText: { fontSize: 14, color: '#366d80ff', fontWeight: '600' },
  emptyState: { alignItems: 'center', paddingVertical: 60 },
  emptyText: { fontSize: 18, color: '#999', marginTop: 16 },
  emptySubtext: { fontSize: 14, color: '#bbb', marginTop: 8 },
  fab: { position: 'absolute', right: 20, bottom: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: '#366d80ff', justifyContent: 'center', alignItems: 'center', elevation: 8 },
  ...
  createButton: { backgroundColor: '#366d80ff' },
  cancelButtonText: { color: '#666', fontSize: 16, fontWeight: '600' },
  createButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  qrModalContent: { backgroundColor: '#fff', borderRadius: 16, padding: 24, alignItems: 'center', width: '90%' },
  qrModalTitle: { fontSize: 24, fontWeight: 'bold', marginBottom: 8 },
  qrBatchId: { fontSize: 16, color: '#666', marginBottom: 24 },
  qrCodeContainer: { padding: 16, backgroundColor: '#fff', borderRadius: 8 },
  qrProductType: { fontSize: 18, fontWeight: 'bold', marginTop: 24 },
  qrQuantity: { fontSize: 14, color: '#666', marginTop: 4 },
  qrCloseButton: { backgroundColor: '#3a93acff', paddingHorizontal: 32, paddingVertical: 12, borderRadius: 8, marginTop: 24 },
