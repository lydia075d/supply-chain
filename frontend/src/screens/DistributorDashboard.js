...
const DistributorDashboard = ({ navigation }) => {
  ...
  // FIX: fetch real checkpoint history from API
  const loadRecentScans = async () => {
    ...
    try {
      ...
      // Only fall back to empty — no fake demo data
      setRecentScans([]);
    } finally {
  ...
  const handleBarCodeRead = async ({ data }) => {
    ...
    try {
      ...
      if (!batchId) {
        Alert.alert('Invalid QR Code', `QR code is missing a batchId.\n\nData: ${JSON.stringify(batchData, null, 2)}`);
        setScanning(false);
    ...
    try {
      ...
      setShowScanner(false);
      // Reload scans to show the new entry
      loadRecentScans();
    } catch (apiError) {
      Alert.alert('Failed to Record Checkpoint', apiError.message);
      setShowScanner(false);
    } finally {
      setScanning(false);
    }
  ...
  const recordCheckpoint = async (batchId, location) => {
    ...
    console.log('[API] Sending checkpoint:', JSON.stringify(checkpointData, null, 2));
    const result = await ApiService.recordCheckpoint(checkpointData);
    console.log('[API] Checkpoint response:', JSON.stringify(result, null, 2));
